...
## Project: cre-design-system

A **pnpm monorepo** with three packages:

| Package | Purpose |
|---|---|
| `packages/cre-web-ui` | React component library. Main deliverable. |
| `packages/cre-unity-bridge` | Loads Unity WebGL builds inside React + bidirectional messaging |
| `apps/storybook` | Storybook 8 (Vite). Deploys to GitHub Pages on push to `main`. |

### The Token-First Constraint
All styling in `@cre/web-ui` flows through the token system. No hardcoded colors, spacing, or typography values are allowed anywhere in component code. The pipeline is:

```
Figma JSON → rawTokens.ts → tokens.ts → cssVars.ts → CreThemeProvider → CSS vars → components
```

**The design system is still incomplete.** Many tokens and semantic roles do not exist yet. When a component needs a value that has no token, the right action is to **extend the token system**, not bypass it. Tasks that touch styling must account for this.

### Key Docs (read these before planning tasks)
- `docs/architecture.md` — system overview, tech stack, decisions, constraints
- `docs/context/token-system.md` — full token pipeline, layer descriptions, how to add tokens
- `docs/context/component-patterns.md` — component structure rules, styling rules, public API conventions

### Folder AGENTS.md files created
- `packages/cre-web-ui/AGENTS.md` — token-first rules, injectStyles, variant patterns
- `apps/storybook/AGENTS.md` — story structure, MDX pairing, forbidden patterns
